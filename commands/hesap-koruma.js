   ...
   switch(hesapkorumaSystemTrue) {
    case "ac": {
      ...
      if (hesapkorumaSystem && hesapkorumaSystemDate) {
          ...
          .setDescription(`<:x_:1211729272364990465> | Bu sistem <t:${parseInt(hesapkorumaSystemDate.date / 1000)}:R> önce açılmış!`)
      ...
      return interaction.reply({ content: "<:tik1:1211728664123674734> | Başarılı bir şekilde sistem açıldı!" });
    }

    case "kapat": {
      if(!hesapkorumaSystem) return interaction.reply({ content: "<:x_:1211729272364990465> | Bu sistem zaten kapalı?" });

      db.delete(`hesapkoruma_${interaction.guild.id}`)
      db.delete(`hesapkorumaDate_${interaction.guild.id}`)
      return interaction.reply({ content: "<:tik1:1211728664123674734> | Başarılı bir şekilde sistem kapatıldı!" });
    }
  }
